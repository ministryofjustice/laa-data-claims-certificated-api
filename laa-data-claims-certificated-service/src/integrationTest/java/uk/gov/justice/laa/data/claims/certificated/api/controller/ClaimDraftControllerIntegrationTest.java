...
@Transactional
class ClaimDraftControllerIntegrationTest extends BaseIntegrationTest {
  @Autowired private ClaimDraftRepository claimDraftRepository;
  ...
  void shouldCreateClaimDraft() throws Exception {
    ...
    MvcResult result =
        mockMvc
            ...
            .andExpect(
                header()
                    .string(
                        "Location", Matchers.startsWith("http://localhost/api/v1/claim-drafts/")))
    ...
    ClaimDraftEntity dbEntity = claimDraftRepository.findById(createdDraftId).orElseThrow();
  ...
  @Test
  @DisplayName(
      "POST /api/v1/claim-drafts with missing required field (sourceSystem) returns 400 Bad Request")
  void shouldReturnBadRequestForMissingRequiredFields() throws Exception {
    ...
    assertThat(claimDraftRepository.count()).isEqualTo(0);
  }

  @Test
  @DisplayName("POST /api/v1/claim-drafts with malformed JSON returns 400 Bad Request")
